...
function onBtn(e) {
    e.preventDefault();
    for(let position1 = 1; position1 <= amount.value; position1 += 1){
        createPromise(position1, delay.value).then(({ position , delay: delay2  })=>{
            console.log(`✅ Fulfilled promise ${position} in ${delay2}ms`);
        }).catch(({ position , delay: delay3  })=>{
            console.log(`❌ Rejected promise ${position} in ${delay3}ms`);
        });
